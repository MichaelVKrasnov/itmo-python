...
def add_add_subparser(subparsers) -> None:
    ...
    add_subparser.set_defaults(func=cmd_add)
...
def add_commit_tree_subparser(subparsers) -> None:
    ...
    commit_tree_subparser.add_argument("-a", dest="author", help="Author's name and email")
...
def add_update_ref_subparser(subparsers) -> None:
    ...
    update_ref_subparser.add_argument("ref", help="Ref")
    update_ref_subparser.add_argument("newvalue", help="New value")
    update_ref_subparser.set_defaults(func=cmd_update_ref)


def add_rev_parse_subparser(subparsers) -> None:
    rev_parse_subparser = subparsers.add_parser("rev-parse", help="Resolve ref")
    rev_parse_subparser.add_argument("rev", help="Ref")
...
def add_symbolic_ref_subparser(subparsers) -> None:
    ...
    symbolic_ref_subparser.add_argument("name", help="Name")
    symbolic_ref_subparser.add_argument("ref", help="Ref")
    symbolic_ref_subparser.set_defaults(func=cmd_symbolic_ref)


def add_checkout_subparser(subparsers) -> None:
    checkout_subparser = subparsers.add_parser("checkout", help="Switch branches or restore working tree files")
    checkout_subparser.add_argument("obj_name", help="Branch or commit name")
    checkout_subparser.set_defaults(func=cmd_checkout)


def add_commit_subparser(subparsers) -> None:
    checkout_commit = subparsers.add_parser("commit", help="Record changes to the repository")
    checkout_commit.add_argument("message", help="Message")
    checkout_commit.add_argument("-a", dest="author", help="Author's name and email")
    checkout_commit.set_defaults(func=cmd_commit)
